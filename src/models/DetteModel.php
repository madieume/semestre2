...
class DetteModel {
    ...
    public function findDetteByTel(string $Tel) {
        $sql="SELECT d.*,cl.*,SUM(montantpaie) verse,(d.montantdet - SUM(p.montantpaie)) AS restant FROM `dette`d JOIN client cl on d.idClient=cl.idcl JOIN paiement p on d.iddet=p.idDette GROUP BY cl.idcl HAVING verse >0 AND cl.telephone='$Tel'";
        $pdo= new \PDO('mysql:host=localhost;dbname=semestre2', "root", "");
        $pdo->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);
        $pdo->setAttribute(\PDO::ATTR_DEFAULT_FETCH_MODE, \PDO::FETCH_OBJ);
        $result=$pdo->query($sql);
